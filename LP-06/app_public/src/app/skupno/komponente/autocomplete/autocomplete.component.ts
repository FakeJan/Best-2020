...
export class AutocompleteComponent implements OnInit {
  ...
  getTagValue(): any {
    let el: any = document.getElementById("tagInput");
    return el.value
  }
  ...
  addTag(): void {
    var tagValue: string = this.getTagValue()?.trim();

    if (!tagValue)
      return;

    this.resetTagValue();
    this.addTagEvent.emit(tagValue);
  }
  ...
  ngOnInit(): void {
    this.autocomplete(document.getElementById("tagInput"), this.allTags);
  }



  private autocomplete(inp: any, listOfValues: string[]): void {
    ...
    inp.addEventListener("input", (e: any) => {
      var a: any, b: any, val: any = inp.value;
      ...
      var uppercased = this.currentTags.map(tag => tag.toUpperCase());
      ...
      listOfValues.forEach(element => {
        if (currentElements <= maxElements) {
          ...
          if (matches && !uppercased.includes(element.toUpperCase())) {
            currentElements++;
      ...
      if (lastElement != null && !uppercased.includes(val.trim().toUpperCase())) {
        b = document.createElement("DIV");
